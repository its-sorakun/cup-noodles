...
async function renderSetup() {
          ...
          <div class="relative z-10 w-full">
            <div class="mb-8">
              <h2 class="weeb-title text-3xl sm:text-4xl flex flex-col gap-2">
                <span>Welcome! ~</span>
                <span class="text-xl sm:text-2xl opacity-40 font-bold">ようこそ！</span>
              </h2>
              <p class="weeb-subtitle">
                Set up your personal streaming vault<br>
              </p>
            </div>
            
            <form id="setup-form" class="space-y-6">
              <div class="weeb-input-group group/input">
                <input type="text" id="setup-username" required class="weeb-input peer" placeholder="Create Username" />
                <label for="setup-username" class="weeb-label">Create Username</label>
              </div>
              
              <div class="weeb-input-group group/input">
                <input type="password" id="setup-password" required class="weeb-input peer" placeholder="Create Password" />
                <label for="setup-password" class="weeb-label">Create Password</label>
              </div>

              <div id="setup-error" class="text-red-400 text-sm text-center hidden font-medium py-2 rounded bg-red-500/10 border border-red-500/20"></div>
  ...
  document.getElementById("setup-form").addEventListener("submit", async (e) => {
    ...
    try {
      ...
      err.classList.remove("hidden");
      btn.disabled = false;
      btn.innerHTML = "Initialize Vault";
    }
  });
}
